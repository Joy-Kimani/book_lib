...
//create new book
export const createBookService = async (author:string,title:string,yop:string,genre:string): Promise<string> => {
    const db = getDbConnectPool(); 
    const result = await db.request()
        .input('author', author)
        .input('title',title)
        .input('yop',yop)
        .input('genre', genre)
        .query('INSERT INTO book (author,title,yop, genre) OUTPUT INSERTED.* VALUES (@author, @title, @yop, @genre)');
