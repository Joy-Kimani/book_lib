...
export const createBooks = async (c: Context) => {
    ...
    try{
        const result = await bookService.createBookService(body.author,body.title, body.yop, body.genre, body.book_id)
        if(result === "Book creation sucess"){
    ...
    try{
       const checkIfBookExists = await bookService.getBookByIdService(book_id);
        ...
        if(result === "Book update Success"){
            return c.json({message:"Successfully updated todo"},500)
        }
